...
function load_config() {
    ...
    $config['driver'] = $params['connection']['type'];
    $config['host'] = $params['connection']['host'];
    $config['username'] = $params['connection']['username'];
    $config['password'] = $params['connection']['password'];
    $config['database'] = $params['connection']['database'];
    $config['mail_server'] = $params['mail']['mailserver'];
    $config['mail_email'] = $params['mail']['email'];
    $config['mail_user'] = $params['mail']['username'];
    $config['mail_password'] = $params['mail']['password'];
}
...
function set_redirect_urls($key, $form_slug, &$ok_url, &$error_url) {
    ...
    if (count($results) == 1) {
        if ($results[0]->urlOnOK != NULL) {
            $ok_url = $results[0]->urlOnOK;
        }
        
        if ($results[0]->urlOnError != NULL) {
            $urlOnError = $results[0]->urlOnError;
        }
    }
}
